...
FIXED VERSION v2 - with proper error handling
"""
...
def main():
    ...
    def detect_ground_mask(image):
        hsv = cv2.cvtColor(image, cv2.COLOR_BGR2HSV)
        h, w = image.shape[:2]
        mask_green = cv2.inRange(hsv, np.array([25, 20, 20]), np.array([85, 255, 255]))
        mask_brown = cv2.inRange(hsv, np.array([10, 30, 20]), np.array([25, 255, 200]))
        ...
        try:
            ...
            if seg is not None:
                ...
                if mode == "skin":
                    ...
                    combined = cv2.inRange(hsv, np.array([90, 30, 80]), np.array([130, 255, 255]))
                else:
            ...
            traceback.print_exc()
            raise HTTPException(500, f"Segment error: {type(e).__name__}: {e}")
        ...
        try:
            ...
            if req.fill_type == "whiten":
                ...
                if enh:
                    ...
                    result = (result + mask_3ch * 60).clip(0, 255).astype(np.uint8)
            ...
            traceback.print_exc()
            raise HTTPException(500, f"Inpaint error: {type(e).__name__}: {e}")
    ...
    async def enhance(req: EnhanceRequest):
        try:
            ...
            filter_name = getattr(req, 'filter', None)

            if filter_name:
                if enh:
                    result = enh.apply_filter(img, filter_name)
                else:
                    result = img
                ...
                if enh:
                    ...
                    if req.saturation != 0:
                        ...
                        hsv[:, :, 1] = np.clip(hsv[:, :, 1] * (1.0 + req.saturation), 0, 255)
            ...
            traceback.print_exc()
            raise HTTPException(500, f"Enhance error: {type(e).__name__}: {e}")
    ...
    async def sky_replace(req: SkyReplaceRequest):
        try:
            ...
            traceback.print_exc()
            raise HTTPException(500, f"Sky replace error: {type(e).__name__}: {e}")
    ...
    async def relight(image_id: str = Form(...), brightness: float = Form(0.3), warmth: float = Form(0.1)):
        try:
            ...
            traceback.print_exc()
            raise HTTPException(500, f"Relight error: {type(e).__name__}: {e}")
